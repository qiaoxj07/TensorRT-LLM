    ...
    def _build_pool_mapping_tensors(self):
        ...
        if self.enable_swa_scratch_reuse:
            ...
            for pool_id in range(self.num_pools):
                ...
                role_a, _ = self._get_pool_roles(pool_id)
                index_base_addr = self.impl.get_mem_pool_base_address(
                    layer_id, role_a, PageIndexMode.SHARED
                )
                kv_cache_pool_pointers_list.append([index_base_addr, 0])
                if self.dtype == DataType.NVFP4:
                    block_scale_role = self._get_block_scale_role(role_a)
                    if block_scale_role is None:
                        block_scale_pool_pointers_list.append([0, 0])
                    else:
                        # The index/scale pointers are a (rep-layer, 0-offset) origin
                        # against which each layer's kv_cache_pool_mapping offset is
                        # resolved. The block-scale origin must reproduce the SAME
                        # per-layer offset() as the index role, so mirror its base for
                        # the same representative layer and shift it back by that
                        # layer's offset. For the base manager offset(rep) == 0, so this
                        # is just the rep layer's scale base; for address-ranked
                        ...
                        # _get_block_scale_role() only returns non-None for Role.KEY,
                        # so _kv_pool_mapping_offset() is the matching offset here.
                        rep_offset = self._kv_pool_mapping_offset(
                            layer_id, pool_id, index_base_addr
                        )
                        scale_stride = (
