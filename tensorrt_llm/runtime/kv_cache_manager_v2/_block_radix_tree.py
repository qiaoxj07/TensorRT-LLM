...
class Block:
    ...
    def __init__(self, tokens: Sequence[TokenIdExt], prev: "Block | RootBlock") -> None:
        ...
        # Covered siblings are no longer removed here; removal is deferred to
        # remove_redundant_covered_siblings(), which runs once the replacement
        # state has actually been committed. That also subsumes the "keep the
        # RootBlock attached while covered children are replaced" ordering fix:
        # by then this block is already in prev.next, so detach_next() can never
        # prune an otherwise-empty root.
